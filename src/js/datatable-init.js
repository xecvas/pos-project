...
export function initDataTable() {
  // Initialize a test DataTable (if required)
  $("#testing").DataTable({});

  // Initialize the main DataTable
  window.myTable = $("#mymenu").DataTable({
    columnDefs: [
  ...
  // Dropdown selection handler for filtering
  $(document).on("click", ".dropdown-item", function (e) {
    e.preventDefault();

    const selectedText = $(this).text(); // Get selected dropdown text
    ...
    window.myTable.column(3).search(filterValue).draw(); // Adjust column index as needed
  });
}
